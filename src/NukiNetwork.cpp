...
void NukiNetwork::initialize() {
  ...
  if (!disableNetwork) {
    ...
    _apiBridgePath[strnlen(api_path_bridge, sizeof(_apiBridgePath) - 1)] = '\0';
...
int NukiNetwork::testNetworkServices() {
  ...
  // 6. return error code
  if (webServerOk && httpClientOk) return 0;    // all OK
  if (!webServerOk && httpClientOk) return -1;  // _server not reachable
  if (webServerOk && !httpClientOk) return -2;  // _httpClient / Home Automation not reachable
...
bool NukiNetwork::update() {
  ...
  if (isConnected()) {
    if (ts - _lastNetworkServiceTs > 30000) {  // test all 30 seconds
      _lastNetworkServiceTs = ts;
      _networkServicesState = testNetworkServices();
      if (_networkServicesState) {  // error in networ Services
...
bool NukiNetwork::connect() {
  ...
  if (_networkDeviceType == NetworkDeviceType::WiFi) {
    ...
    if (_preferences->getBool(preference_find_best_rssi, false)) {
      for (int i = 0; i < _foundNetworks; i++) {
        if (ssid == WiFi.SSID(i)) {
          Log->println(String("Saved SSID ") + ssid + String(" found with RSSI: ") + String(WiFi.RSSI(i)) + String(("(")) + String(constrain((100.0 + WiFi.RSSI(i)) * 2, 0, 100)) + String(" %) and BSSID: ") + WiFi.BSSIDstr(i) + String(" and channel: ") + String(WiFi.channel(i)));
          if (bestConnection == -1) {
            bestConnection = i;
          } else {
      ...
      if (bestConnection == -1) {
        ...
        Log->println(String("Trying to connect to SSID ") + ssid + String(" found with RSSI: ") + String(WiFi.RSSI(bestConnection)) + String(("(")) + String(constrain((100.0 + WiFi.RSSI(bestConnection)) * 2, 0, 100)) + String(" %) and BSSID: ") + WiFi.BSSIDstr(bestConnection) + String(" and channel: ") + String(WiFi.channel(bestConnection)));
      }
    }
...
void NukiNetwork::onRestDataReceivedCallback(const char* path, WebServer& server) {
  ...
  if ((_networkServicesState == -1) || (_networkServicesState == -3)) {
    return;
  }
  ...
  onRestDataReceived(path, server);

  for (auto receiver : _restDataReceivers) {
...
void NukiNetwork::onRestDataReceived(const char* path, WebServer& server) {
  ...
  if (comparePrefixedPath(path, api_path_bridge_enable_api)) {
    if (server.hasArg("enable") && (server.arg("enable").toInt() == 0 || server.arg("enable").toInt() == 1)) {
      ...
      preferences->putBool(preference_api_enabled, _apiEnabled);
      sendResponse(json);
    } else {
...
const int8_t NukiNetwork::signalStrength() {
...
void NukiNetwork::buildApiPath(const char* path, char* outPath) {
  ...
  strncpy(outPath, _apiPath, sizeof(_apiPath) - 1);
  outPath[strnlen(_apiPath, sizeof(_apiPath) - 1)] = '\0';  // Null-Terminierung sicherstellen
