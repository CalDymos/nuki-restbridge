...
// pClient->setConnectionParams(12,12,0,600,64,64);
  ...
  PairingResult NukiBle::pairNuki(AuthorizationIdType idType)
  {
    ...
    if (pairingLastSeen < millis() - 2000)
      pairingServiceAvailable = false;
#else
    if (pairingLastSeen < (esp_timer_get_time() / 1000) - 2000)
      pairingServiceAvailable = false;
#endif
  ...
  Nuki::CmdResult NukiBle::retrieveKeypadEntries(const uint16_t offset, const uint16_t count)
  {
    ...
    if (result == Nuki::CmdResult::Success)
    {
      ...
      {
#ifndef NUKI_64BIT_TIME
        if (millis() - timeNow > generalTimeoutDuration)
        {
#else
...
// wait for return of Keypad Codes (0x0045)
  ...
  void NukiBle::saveCredentials()
  {
    ...
    if ((preferences.putBytes(BLE_ADDRESS_STORE_NAME, currentBleAddress, 6) == 6) && (preferences.putBytes(SECRET_KEY_STORE_NAME, secretKeyK, 32) == 32) && (preferences.putBytes(AUTH_ID_STORE_NAME, authorizationId, 4) == 4))
    {
      if (debugNukiConnect)
      {
  ...
  bool NukiBle::retrieveCredentials()
  {
    ...
    if ((preferences.getBytes(BLE_ADDRESS_STORE_NAME, buff, 6) > 0) && (preferences.getBytes(SECRET_KEY_STORE_NAME, secretKeyK, 32) > 0) && (preferences.getBytes(AUTH_ID_STORE_NAME, authorizationId, 4) > 0))
    {
